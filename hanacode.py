...
def Ad1SSRefl(y):
    ...
    Ad1res = np.sum(odeint(lmodelef, [Ad1y[0], 0, 0, y[3], 0], Ad1x, args=(λ, K, β, V, k, δ, p, ε, c, α))[:, 0:3],
                    axis=1)
    # ary = ((odeint(basemodel, PBSy[0], PBSx, args = (λ, K)).reshape(15,) - PBSy)**2)
    # SSR1 = sum(ary)
...
Ad1fit = scipy.optimize.minimize(Ad1SSRefl, [1.21264372e-01, 1.34682793e+03, 1.39195274e+01, 1.48402225e-02,
                                             ...
                                             1.32574167e-01, 4.06501041e-08], method='Nelder-Mead',
                                 bounds=([[0, None], [0, None], [0, None], [0, None],
                                          [0, None], [0.01, None], [0.00, None], [0.00, None], [0.00, None],
                                          [0.00, None]]))
...
Ad1res = np.sum((odeint(lmodelef, [Ad1y[0], 0, 0, Ad1fit.x[3], 0], Ad1x,
                        args=(Ad1fit.x[0], Ad1fit.x[1], Ad1fit.x[2], Ad1fit.x[3],
                              Ad1fit.x[4], Ad1fit.x[5], Ad1fit.x[6], Ad1fit.x[7], Ad1fit.x[8], Ad1fit.x[9])))[:, 0:3],
                axis=1)
...
# resid immune
residAd1 = np.sum((odeint(emodelef, [Ad1y[0], 0, 0, 8.79257095e-03, 0], Ad1x,
                          args=(1.45082670e+01, 8.79259920e-03, 7.40647857e-02, 7.40642482e-02,
